...
fn multi_threaded_crossbeam(iteration_size: usize) {
    use crossbeam::queue::ArrayQueue;
    const SIZE: usize = 1024 * 16;
    let rb: Arc<ArrayQueue<usize>> = Arc::new(ArrayQueue::new(SIZE));
    let sum = Arc::new(AtomicUsize::new(0));
    let sum_err = Arc::new(AtomicUsize::new(0));
    let mut threads = vec![];
...
fn criterion_benchmark(c: &mut Criterion) {
    let mut group = c.benchmark_group("Native vs Crossbeam");
    ...
    for i in [20usize, 21usize].iter() {
        group.bench_with_input(BenchmarkId::new("multi-threaded usize", i), i, |b, _i| {
            b.iter(|| multi_threaded(black_box(iteration_size)))
        });
        group.bench_with_input(BenchmarkId::new("crossbeam usize", i), i, |b, _i| {
            b.iter(|| multi_threaded_crossbeam(black_box(iteration_size)))
        });
    }
    group.finish();
}
